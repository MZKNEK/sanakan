...
{
    public class RequireAnyCommandChannelLevelOrNitro : PreconditionAttribute
    {
        private readonly long _level;

        public RequireAnyCommandChannelLevelOrNitro(long level) => _level = level;
        ...
        public async override Task<PreconditionResult> CheckPermissionsAsync(ICommandContext context, CommandInfo command, IServiceProvider services)
        {
            ...
            using (var db = new Database.DatabaseContext(config))
            {
                ...
                if (gConfig.NitroRole != 0)
                {
                    if (user.Roles.Any(x => x.Id == gConfig.NitroRole))
                        return PreconditionResult.FromSuccess();
                }
