    ...
    public class CardsQueryFilter
    {
        ...
        public static IOrderedQueryable<Card> Use(OrderType type, IQueryable<Card> query)
        {
            var orderedQuery = type switch
            {
                ...
                OrderType.Picture => query.OrderBy(x => x.CustomImage == null ? (x.Image == null ? 0 : 1) : (x.IsAnimatedImage ? 3 : 2)),
                OrderType.PictureDes => query.OrderByDescending(x => x.CustomImage == null ? (x.Image == null ? 0 : 1) : (x.IsAnimatedImage ? 3 : 2)),
